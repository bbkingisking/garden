...
PASSWORD_CMD="sudo systemd-creds decrypt --name sonnets-db-backup $CREDS/sonnets-db-backup.cred"
...
sudo cp "$SCRIPT_DIR/bin/sonnets-backup.sh" /usr/local/sbin/sonnets-backup
sudo chmod +x /usr/local/sbin/sonnets-backup
...
sudo cp "$SCRIPT_DIR/systemd/sonnets.service"        /etc/systemd/system/
sudo cp "$SCRIPT_DIR/systemd/sonnets.timer"          /etc/systemd/system/
sudo cp "$SCRIPT_DIR/systemd/sonnets-backup.service" /etc/systemd/system/
sudo cp "$SCRIPT_DIR/systemd/sonnets-backup.timer"   /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now sonnets.timer
sudo systemctl enable --now sonnets-backup.timer
...
RESTIC_REPO=$(sudo systemd-creds decrypt --name sonnets-db-url $CREDS/sonnets-db-url.cred)
...
RESTORED_DUMP=$(find "$TMPDIR" -name "sonnets.sql" -type f)
